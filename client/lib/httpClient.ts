...
/**
 * Most-recent API failure, kept (only while App Debug is on) so error toasts can
 * surface the real status/message/field-errors that caused them. Correlated by
 * time — see getRecentApiError.
 */
type RecentApiError = {
  method: string;
  url: string;
...
let lastApiError: RecentApiError | null = null;

/** The last API error, if one happened within `withinMs` (App Debug only). */
export function getRecentApiError(withinMs = 6000): RecentApiError | null {
  if (!lastApiError) return null;
  return Date.now() - lastApiError.at <= withinMs ? lastApiError : null;
}
...
async function request<T>(
  ...
  try {
    ...
    // App Debug: surface network-level failures (server down, CORS, DNS…).
    if (isAppDebug()) {
      console.error(
        `%c[API ✗ network] ${method} ${url}`,
  ...
  if (!response.ok) {
    ...
    // App Debug: log the full request/response, and remember it briefly so
    // the error toast can show the underlying cause.
    if (isAppDebug()) {
      lastApiError = {
        method,
        url,
      ...
      console.error(
        ...
        "color:#dc2626;font-weight:bold",
        { message, fieldErrors, payload: result },
      );
    }
...
export function applyApiErrorToForm<T extends FieldValues>(
  ...
  if (err instanceof ApiError && err.fieldErrors.length > 0) {
    ...
    for (const { property, message } of err.fieldErrors) {
      const isKnown = knownFields ? (knownFields as string[]).includes(property) : true;
